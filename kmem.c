...
int pcidriver_kmem_alloc(pcidriver_privdata_t *privdata, kmem_handle_t *kmem_handle)
{
    ...
    if (!pci_set_dma_mask(privdata->pdev, DMA_BIT_MASK(64))) {
        pci_set_consistent_dma_mask(privdata->pdev, DMA_BIT_MASK(64));
        printk("using a 64-bit irq mask\n");
    } else {
        printk("unable to use 64-bit irq mask\n");
        return ;
    }
	 ...
	 */
#if 0
	dma_addr_t hw_dma_addr = 0;
	retptr = dma_alloc_coherent( &(privdata->pdev->dev), kmem_handle->size, &hw_dma_addr, GFP_KERNEL);
	
	//kmem_entry->dma_handle = dma_map_single( &(privdata->pdev->dev), retptr, kmem_handle->size, DMA_BIDIRECTIONAL);
	kmem_entry->dma_handle = hw_dma_addr;
#else
	dma_addr_t hw_dma_addr = 0;
	while(1) {
		retptr = pci_alloc_consistent( privdata->pdev, kmem_handle->size, &(kmem_entry->dma_handle) );
		printk("retptr(0x%llx) dma_handle(0x%llx) debug_dma_addr(0x%llx) \r\n",retptr, kmem_entry->dma_handle, hw_dma_addr);		
		if((kmem_entry->dma_handle)>>32 !=0) {break;}
	}
#endif
	printk("retptr(0x%llx) dma_handle(0x%llx) debug_dma_addr(0x%llx) \r\n",retptr, kmem_entry->dma_handle, hw_dma_addr);
	if (retptr == NULL)
	...
	mod_info("malloc kernel vir:0x%x phy:0x%x\n",retptr, kmem_entry->dma_handle);
...
int pcidriver_mmap_kmem(pcidriver_privdata_t *privdata, struct vm_area_struct *vma)
{
	...
	struct list_head *tail = privdata->kmem_list.prev;
	kmem_entry = list_entry(tail, pcidriver_kmem_entry_t, list);
	spin_unlock(&(privdata->kmemlist_lock));
...
//	vma->vm_page_prot = pgprot_noncached(vma->vm_page_prot);
    ...
    spin_lock(&(privdata->kmemlist_lock));
	list_move_tail(tail, &privdata->kmem_list_mmaped);
	spin_unlock(&(privdata->kmemlist_lock));
