...
static int pcidriver_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	else if ((id->vendor == PCIE_SC_VENDOR_ID) &&
		(id->device == PCIE_SC_DEV_ID))
	{
                /* It is a PCI-E Creotech uTCA AMC board */
		mod_info("Found Altera A10 board at %s\n", dev_name(&pdev->dev));
	}
	else
	{
    ...
    INIT_LIST_HEAD(&(privdata->kmem_list_mmaped));
...
int pcidriver_release(struct inode *inode, struct file *filp)
{
    ...
    list_for_each_safe(entry, tmp, &privdata->kmem_list_mmaped) {
        pcidriver_kmem_entry_t *kmem_entry = list_entry(entry, pcidriver_kmem_entry_t, list);
        list_move_tail(entry, &list);
	    kmem_count++;
    }
    spin_unlock(&(privdata->kmemlist_lock));

    atomic_sub(kmem_count, &privdata->kmem_count);
